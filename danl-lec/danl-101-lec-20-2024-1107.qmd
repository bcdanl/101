...
- `geom_bar()` creates a bar chart.
  - We map the `x` or `y` aesthetic to the variable.
...
## Bar Chart with `geom_bar()`
### `count(CATEGORICAL)` the occurrences of each category in CATEGORICAL 
:::{.nonincremental}
...
- We can create a stacked bar chart by mapping the `fill` aesthetic to other CATEGORICAL variable.
...
diamonds |>
  ...
  theme_void() +
  # geom_text(aes(y = ypos, label = paste0(cut, "\n (", round(prop, 1), "%)")), 
